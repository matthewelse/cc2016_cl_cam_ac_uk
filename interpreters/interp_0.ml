...
*)
let update (env, (x, v)) y = if x = y then v else env y
...
let rec interpret (e, env, store) =
  ...
  | While (e1, e2) -> (
      ...
      | _v -> complain "runtime error.  Expecting a boolean!" )
  ...
  | Fst e -> (
    ...
    | _v, _ -> complain "runtime error.  Expecting a pair!" )
  | Snd e -> (
    match interpret (e, env, store) with
  ...
  | Case (e, (x1, e1), (x2, e2)) -> (
      ...
      | _v -> complain "runtime error.  Expecting inl or inr!" )
  | Lambda (x, e) ->
  ...
  | App (e1, e2) -> (
      ...
      match v1 with
      | FUN f -> f (v2, store2)
      | _v -> complain "runtime error.  Expecting a function!" )
  | LetFun (f, (x, body), e) ->
      let new_env =
        update
          ( env
...
(* env_empty : env *)
let empty_env x = complain (x ^ " is not defined!\n")

(* store_empty : env *)
let empty_store x = complain (string_of_int x ^ " is not allocated!\n")
