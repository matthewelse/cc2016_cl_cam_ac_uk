...
let i2 options expr =
  ...
  |> Interp_2.interpret options |> Interp_2.string_of_value

let i3 options expr =
  Backend.Compiler_3.compile options expr
  |> Interp_3.interpret options |> Common.Interp_3.string_of_value
...
let run =
  ...
  and verbose_front =
    flag "-verbose-front" no_arg ~doc:"FLAG verbose front-end"
  and verbose_tree = flag "-verbose-tree" no_arg ~doc:"FLAG verbose tree"
  and verbose_vm =
    flag "-verbose-vm" no_arg ~doc:"FLAG verbose virtual machine"
  and stack_max =
    flag "-stack-max" (optional_with_default 1000 int) ~doc:"INT max heap size"
  and heap_max =
    flag "-heap-max" (optional_with_default 1000 int) ~doc:"INT max heap size"
  in
  let options : Options.t =
    {verbose_front; verbose_tree; verbose_back; verbose_vm; stack_max; heap_max}
  in
